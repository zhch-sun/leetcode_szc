...
if __name__ == '__main__':
    ...
    解法1:
        ...
        while n: 正常思路, 可以str +=
        也可以先算出最终的阶乘，循环中每次除n: 没有变得太快
    解法2:
        while k可以实现early stopping. 
        k=0的时候只要按顺序把剩下的数字放进去即可
        这样最好用一个list暂存. 
        但其实不快. 
    corner case:
        python 没有阶乘!  ...  只能math.factorial. 
        还有就是k和n的对齐. 因为k=1的时候对应是不选的, 所以k-=1!
    TODO Note: 目前所有的做法都需要从一个list里面remove.
        要么就是平移所有的数..其实就是remove.
    """
    s = Solution()
